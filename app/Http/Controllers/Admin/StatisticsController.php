...
class StatisticsController extends Controller
{
    public function index()
    {
        ...
        return view('admin.statistics.index', compact('monthlyRevenue', 'growthRates', 'bestSellingProducts', 'leastSellingProducts'));
    }
    ...
    public function showStatistics(Request $request)
    {
        ...
        $monthlyRevenue = Bill::selectRaw('DATE_FORMAT(created_at, "%Y-%m") as month, SUM(total) as total_revenue')
        ...
        $bestSellingProducts = BillDetail::select('product_name', DB::raw('SUM(quantity) as total_sold'), DB::raw('SUM(quantity * price) as total_revenue'), DB::raw('ROUND(SUM(quantity * price) / ' . ($totalRevenue > 0 ? $totalRevenue : 1) . ' * 100, 2) as revenue_percentage'))
            ->join('bills', 'bill_details.bill_id', '=', 'bills.id')
            ->where('bills.status', 4) // Chỉ lấy đơn hàng đã hoàn tất
            ->whereBetween('bills.created_at', [$startDate, $endDate]) // Lọc theo ngày
        ...
        $leastSellingProducts = BillDetail::select('product_name', DB::raw('SUM(quantity) as total_sold'), DB::raw('SUM(quantity * price) as total_revenue'), DB::raw('ROUND(SUM(quantity * price) / ' . ($totalRevenue > 0 ? $totalRevenue : 1) . ' * 100, 2) as revenue_percentage'))
            ->join('bills', 'bill_details.bill_id', '=', 'bills.id')
            ->where('bills.status', 4) // Chỉ lấy đơn hàng đã hoàn tất
            ->whereBetween('bills.created_at', [$startDate, $endDate]) // Lọc theo ngày
            ->groupBy('product_name')
